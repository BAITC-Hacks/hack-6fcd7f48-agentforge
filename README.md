...
Это примеры записей текущего набора для проверки чтения evidence, не зашитые ответы: поиск доступен и для других `gid`. Локальная UI-приёмка на desktop 1440 px и mobile 390 px покрыла поиск изолированного узла `100000000456947100`, неизвестный `gid`, пустой результат фильтров роли/кластера, выбор узла на графе, zoom/fit, связи в карточке, сводку кластера и предупреждение границы. Страница не переполняет viewport, элементы управления работают. В плотных окрестностях читать граф трудно: узлы и подписи могут пересекаться; улучшение раскладки и масштаба остаётся следующим этапом. Приоритетный score отображается с тремя знаками, а `role_score` — как «Сила признаков роли» в процентах.
...
- Эти результаты относятся к локальной приёмке чистой копии. Удалённый fresh clone и CI ещё не подтверждены; актуальный статус после публикации будет в [GitHub Actions](../../actions/workflows/verify.yml), определение workflow — в [verify.yml](.github/workflows/verify.yml).
