...
Новая раскладка проверена на экранах шириной 1440 и 390 px: поиск, раскрытие всей окрестности, масштабирование и выделение переводов работают без горизонтального переполнения страницы и ошибок JavaScript. Подробные сценарии — в [QA-отчёте](docs/qa-report.md). Большие обзорные графы требуют приближения и выбора отдельных узлов; в плотных областях возможны пересечения связей и подписей. Приоритет отображается с тремя знаками, а `role_score` — как «Сила признаков роли» в процентах.
