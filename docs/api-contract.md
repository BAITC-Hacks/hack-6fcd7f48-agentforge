...
`Node = {gid:string,role:string,role_score:number,cluster_id:number,priority_score:number,priority_breakdown:PriorityBreakdown[],priority_factor:number,priority_reason:string,evidence:string,depth:number,is_seed:boolean,in_degree:number,out_degree:number,in_kzt:number,out_kzt:number,in_tx:number,out_tx:number,pagerank:number,pass_through:number|null,in_concentration:number,out_concentration:number,boundary:boolean,component_id:number,seed_source_count:number,seed_paths:string[][],collecting_branches:number,outgoing_communities:number}`.

`PriorityBreakdown = {label:string,value:number,normalized:number,weight:number}`. `seed_source_count` считает разные seed с монотонными путями до узла, исключая сам узел; для seed это 0; `seed_paths` содержит до трёх кратчайших примеров путей на строго возрастающей глубине и с исключением собственного seed-пути. Эти пути описывают графовую достижимость, не происхождение конкретных средств и не хронологию операций. `collecting_branches` считает непосредственных входящих соседей, которые не являются seed, подходят под collecting rule и имеют меньшую глубину; `outgoing_communities` — число кластеров среди исходящих соседей.
...
Роли: consolidator, distributor, transit, coordinator, terminal, peripheral — в порядке применения правил; peripheral является остаточной ролью. PageRank включён как справочная метрика и не влияет на роль или priority. Русские подписи определяются интерфейсом, значения CSV/API остаются английскими. Карточка и граф дают гипотезы; scores не являются вероятностью виновности. Граница depth=4 и неполнота входящих seed явно показаны.
